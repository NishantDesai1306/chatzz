...
exports.getMissedMessagesFromChatRoom = function(chatRoomId, chatUserId) {
    if (!chatRoomId) {
        return Q.reject(new Error('null chatUser id passed'));
    }
    if (!chatUserId) {
        return Q.reject(new Error('null chatUserId passed'));
    }

    var chatMessageModel = ChatMessage.model();
    return chatMessageModel.getMissedMessages(chatRoomId, chatUserId);
}
